...
function deterministicFoodParse(text){
  const input=norm(text), candidates=[];
  const qtyWords=Object.keys(NUMBER_WORDS).join('|');
  for(const key of KNOWN_KEYS){
    for(const rawAlias of FOODS[key].aliases){
      const alias=norm(rawAlias);
      const re=new RegExp(`(?:\\b(\\d+(?:\\.\\d+)?|${qtyWords})\\s+)?\\b${esc(alias)}\\b`,'g');
      let m; while((m=re.exec(input))){candidates.push({key,alias,quantity:parseQty(m[1]),start:m.index,end:re.lastIndex});}
    }
  }
  // Prefer the match that starts earliest; for the same start prefer the longest phrase.
  // This prevents "egg" from consuming "eggs" and generic "tortilla" from consuming "tortillas".
  candidates.sort((a,b)=>a.start-b.start || (b.end-b.start)-(a.end-a.start));
  const matches=[],occupied=[];
  for(const c of candidates){if(occupied.some(([a,b])=>c.start<b&&c.end>a))continue;matches.push(c);occupied.push([c.start,c.end]);}
  if(!matches.length)return null;
  matches.sort((a,b)=>a.start-b.start);
  let leftover=input;
  for(const m of [...matches].sort((a,b)=>b.start-a.start))leftover=leftover.slice(0,m.start)+' '+leftover.slice(m.end);
  const unknown=norm(leftover).split(' ').filter(Boolean).filter(w=>!FILLER.has(w)&&!NUMBER_WORDS[w]&&!/^\d+(?:\.\d+)?$/.test(w));
...
export default async function handler(req,res){
 ...
 try{
  const body=typeof req.body==='string'?JSON.parse(req.body):(req.body||{});const text=String(body.text||'').trim().slice(0,1200);const user=String(body.user||'User').slice(0,40);if(!text)return json(res,400,{error:'Missing text'});
